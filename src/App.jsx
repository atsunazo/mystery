        ...
        {activeTab === 'matrix' && (
          <section className="panel-stack">
            <div className="panel panel-header">
              <div>
                <h2>希望マトリックス</h2>
                <p>作品タイトル列だけ固定し、右側と上下を表の中でスクロールできます。</p>
              </div>
            </div>
                ...
                <table className="matrix-table">
                  <thead>
                    <tr>
                      ...
                      <th className="sum-head">×</th>
                      {members.map((member) => (
                        <th key={member.id} className="member-head-cell horizontal-name">{member.name}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={work.id}>
                          <td className="work-title-fixed">
                            <button className="matrix-work-link" onClick={() => { setSelectedWorkId(work.id); setActiveTab('works') }}>
                              {work.title}
                            </button>
                          </td>
                          ...
                          {members.map((member) => (
                            <td key={`${work.id}-${member.id}`} className={`matrix-symbol-cell ${getWorkSymbolClass(member, work.id)}`}>
